...
<!-- Funcion que determina si un String es un numero o no -->
function IsNumeric(sText)
{
    ...
    if (sText.length == 0)
        ...
        for (i = 0; i < sText.length && IsNumber == true; i++) 
            ...
            <!-- Begin
            function isValidDate(dateStr, message,message2) {
                ...
                if ((month==4 || month==6 || month==9 || month==11) && day==31) {
                    alert("Mes "+month+" no tiene 31 dias!")
                    return false;                    
                }
            ...
            function enableFields(form, itemName, imageName, theme) {
                var periodicity_ = document.getElementById('periodicity'+itemName);
                var periodicity = periodicity_.options[periodicity_.selectedIndex].value;
                var notification_ = document.getElementById('notification'+itemName);
                var notification = notification_.options[notification_.selectedIndex].value;
                var format_ = document.getElementById('format'+itemName);
                var format = format_.options[format_.selectedIndex].value;                              
                
                if (form.selectedItems == null)
                    form.selectedItems = new Array();                
                if (MM_arrayContainsString(form.selectedItems, itemName+"-"+periodicity+"-"+notification+"-"+format)) {
                    form.selectedItems = MM_removeStringFromArray(form.selectedItems, itemName+"-"+periodicity+"-"+notification+"-"+format);
                    document[imageName].src = './themes/'+theme+'/checkbox_off_16.gif';                   
                    document.getElementById('periodicity'+itemName).disabled=true;
                    document.getElementById('format'+itemName).disabled=true;
                    document.getElementById('notification'+itemName).disabled=true;                  
                } else {
                form.selectedItems[form.selectedItems.length] = itemName+"-"+periodicity+"-"+notification+"-"+format;
                document[imageName].src = './themes/'+theme+'/checkbox_on_16.gif';
                document.getElementById('format'+itemName).disabled=false;
                document.getElementById('periodicity'+itemName).disabled=false;
                document.getElementById('notification'+itemName).disabled=false; 
            }
        }        
        
        function changeValue(form,itemName) {
    ...
    function isInteger (s)
    {
        ...
        function validateInt(sText)
        {           
           
            if (!isInteger(sText))
                {   
                    alert('Tiempo Estimado no tiene un valor valido');
                    return false;
                }
                return true;
